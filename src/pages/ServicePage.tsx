      ...
      <div className="relative z-10 container mx-auto px-6 max-w-3xl text-center">
        ...
        >
          <motion.div variants={childVariant}>
            <Link
              to={{ pathname: localizedPath("/"), hash: "servicos" }}
          ...
          {(service.price || service.priceLabel) && (
            <motion.div variants={childVariant} className="mt-2">
              <div className="card-mystical inline-block px-8 py-3 text-center">
                ...
                {service.price && (
                  <p className="text-primary font-bold text-3xl font-display">{service.price}</p>
                )}
                {service.duration && (
  ...
  <section className="py-20 bg-gradient-dark">
    <div className="container mx-auto px-6 max-w-4xl">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        >
          <div>
            <h2 className="text-2xl font-display font-bold text-shimmer-gold mb-4">{aboutTitle}</h2>
            <p className="text-foreground/80 font-light leading-relaxed">{service.description}</p>
          </div>

          <div className="card-mystical p-6 glow-border-hover">
            <p className="text-primary font-semibold text-sm uppercase tracking-widest mb-3">{forWhomTitle}</p>
            <p className="text-muted-foreground font-light leading-relaxed">{service.forWhom}</p>
          </div>
        </motion.div>
        ...
        >
          <div className="card-mystical p-8 glow-border-hover h-full">
            <h3 className="text-xl font-display font-bold text-shimmer-gold mb-6">{receivesTitle}</h3>
        ...
        >
          ...
          {service.price && (
            <p className="text-lg text-muted-foreground font-light mb-8">
              {t("servicePage.availableFor", { title: service.title })}{" "}
              <span className="text-primary font-semibold">{service.price}</span>
            </p>
          )}
          {!service.price && service.priceLabel && (
            <p className="text-lg text-muted-foreground font-light mb-8">{service.priceLabel}</p>
          )}
...
const RelatedServicesSection = ({
  ...
  const { localizedPath } = useLocale();
  const related = slugs.map((s) => bySlug[s]).filter(Boolean);
  ...
  return (
    <section className="py-16 bg-gradient-dark">
      <div className="container mx-auto px-6 max-w-4xl">
        ...
        >
          <h3 className="text-2xl font-display font-bold text-shimmer-gold">{t("servicePage.seeAlso")}</h3>
        </motion.div>

        <div
          className={`grid gap-6 ${related.length === 1 ? "grid-cols-1 max-w-sm mx-auto" : "grid-cols-1 sm:grid-cols-2"}`}
        >
          {related.map((s, i) => (
            <motion.div
              key={s.slug}
            ...
            >
              ...
              <h4 className="font-display text-lg font-semibold mb-1 text-foreground">{s.title}</h4>
              <p className="text-muted-foreground text-sm mb-3">{s.subtitle}</p>
              {s.price && <p className="text-primary font-bold text-xl mb-4">{s.price}</p>}
      ...
      <div className="flex min-h-screen items-center justify-center px-6">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-display font-bold text-shimmer-gold mb-4">
            {t("servicePage.notFoundTitle")}
          </h1>
          <p className="text-muted-foreground mb-8">{t("servicePage.notFoundBody")}</p>
          <Link
            to={{ pathname: localizedPath("/"), hash: "servicos" }}
            className="shimmer inline-block bg-gradient-gold text-primary-foreground font-semibold px-8 py-3 rounded-lg"
          >
            {t("servicePage.notFoundCta")}
          </Link>
        </div>
      </div>
      <FooterSection />
  ...
  return (
    <main className="min-h-screen bg-background overflow-x-hidden">
      ...
      <ServiceDetailsSection
        service={service}
        aboutTitle={t("servicePage.aboutTitle")}
        forWhomTitle={t("servicePage.forWhom")}
        receivesTitle={t("servicePage.receivesTitle")}
        durationLabel={t("servicePage.durationLabel")}
      />
      <ServiceCTASection service={service} t={t} />
      {service.relatedSlugs && service.relatedSlugs.length > 0 && (
        <RelatedServicesSection slugs={service.relatedSlugs} bySlug={bySlug} t={t} />
      )}
      <FooterSection />
      <FloatingWhatsApp />
    </main>
